...
#Create and empty matrix with one row and column with the amount of cell_number
...
#create the dataframe consisting of link outflows CAV proportion
df1 = pd.DataFrame({'Outflows_CAV_prop': [0]})
df2 = pd.DataFrame({'Outflows_CAV_prop': CTM_matrix_recap_perc.iloc[:,-1]})

Outflows_CAV_proportion = pd.concat([df1, df2], ignore_index=True)

#add the newly defined dataframe into IO_matrix
IO_matrix['Outflows_CAV_proportion'] = Outflows_CAV_proportion

#visualisation of CAV percentage in the link outflow

#continuous
sns.lineplot(x = IO_matrix.index,y = IO_matrix["Outflow"], color='blue')
...
IO_matrix.to_excel('IO_matrix_MCTM.xlsx', index = True, header = True)